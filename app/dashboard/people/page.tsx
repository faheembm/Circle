...
export default function PeoplePage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 max-w-xl pb-24 md:pb-6">
      <h1 className="font-semibold mb-6">Find people</h1>
            ...
            {results.map((profile) => (
              <div key={profile.id} className="flex items-center gap-3 p-3 rounded-[var(--radius-sm)] hover:bg-[var(--bg-secondary)] transition-colors">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <Link href={`/dm/${profile.id}`} className="btn btn-ghost text-xs px-3 py-1.5">
                    DM
                  </Link>
                  <button
                    className={followState[profile.id] ? 'btn btn-ghost text-xs px-3 py-1.5' : 'btn text-xs px-3 py-1.5'}
                    onClick={() => handleFollow(profile.id)}
                  >
                    {followState[profile.id] ? 'Following' : 'Follow'}
                  </button>
                </div>
              </div>
            ))}
      ...
      {query.length === 0 && (
        <div className="text-center py-12">
          <p className="text-sm" style={{ color: 'var(--text-faint)' }}>
            Search for someone by username or display name
          </p>
        </div>
      )}
    </div>
