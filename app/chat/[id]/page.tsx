  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              {group.is_official && (
                <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'var(--bg-secondary)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>
                  official
                </span>
              )}
            </div>
        ...
        <Link href={`/chat/${id}/members`} className="btn btn-ghost text-xs px-3 py-1.5">
          Members
        </Link>
      </header>
      ...
      {member ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            Join <strong>{group.name}</strong> to see messages
          </p>
          <button className="btn px-6" onClick={handleJoin}>
            Join group
          </button>
        </div>
      )}
